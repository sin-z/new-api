...
| 实际执行结果 | 在 `.worktree/xrtoken-ark-video-adaptor` 执行 `codegraph status` | Project 为 `<workspace>/.worktree/xrtoken-ark-video-adaptor`；Files 1,961、Nodes 29,363、Edges 83,855；`Index is up to date`。 | CodeGraph 可作为本 worktree 的定位辅助证据。 |
